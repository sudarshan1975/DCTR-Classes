namespace DCTRClasses
{
    public class DopplerEngine
    {
        ...
        static readonly double C2 = Math.Sqrt(2 * Math.Log(2));

        const double CDoppler = 1.13246322E-08;

        double _curWNum = -1;

        static readonly double dTerm = Math.Sqrt(2 * Math.PI);

        static Dictionary<string, Dictionary<int, double>> molWtDict = [];

        double _WNumMult = 1;

        #endregion Private Properties

        public static void SetMolecularWeights(string baseFolder)
        {
            molWtDict = [];

            string? curKey = null;

            using StreamReader sr = new(baseFolder + "\\Data\\molecularWeights.dat");
        ...
        /// <returns></returns>
        public static double GetHWHH(double TInKelvin, double wNumInInvCm, string species, int isotopeID)
        {
        ...
        /// <param name="isotopeID"></param>
        /// <returns></returns>
        public static double GetStdDev(double TInKelvin, double wNumInInvCm, string species, int isotopeID)
        {
        ...
        /// <returns></returns>
        public double[]? GetDistributionArray(double TInKelvin,
            double wNum, string species, int isotopeID, double wNumRes)
        {
            ...
            if (_curWNum > 0 && wNum < _curWNum * _WNumMult) return _cachedArray;
            ...
            if (double.IsNaN(stdDev)) return null;
